...
const ProductsList = () => {
 ...
 const handleApprove = async (id) => {
    // 1. DEBUG CHECK: Ensure we actually have an ID
    if (!id) {
        alert("Error: Cannot approve because Product ID is missing.");
        console.error("handleApprove called with undefined ID");
        return;
    }
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let list = products.filter(p => {
      ...
      if (activeTab === 'pending') {
        ...
        // 2. Approvers see items assigned to them
        // --- DEBUGGING LOGIC START ---
        // We check against BOTH the Mongo _id (String) AND the AutoIncrement userId (Number)
        const myMongoId = String(user._id);
        const myNumericId = String(user.userId);

        const isAssignedToMe = p.approvers?.some(appr => {
            const savedId = String(appr.user_id);
            
            // DEBUG LOG: View this in Chrome Console (F12) to see why it fails/succeeds
            console.log(`Checking Product: ${p.product_name?.en || p.product_name}`);
            console.log(`-- Saved Approver ID: ${savedId}`);
            console.log(`-- My Mongo ID: ${myMongoId}`);
            console.log(`-- My Numeric ID: ${myNumericId}`);
            
            // The Fix: Return TRUE if it matches EITHER ID format
            return savedId === myMongoId || savedId === myNumericId;
        });
        // --- DEBUGGING LOGIC END ---
